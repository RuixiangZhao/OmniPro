#!/bin/bash
# =============================================================================
# OmniProact-Bench — Video-SALMONN 2+ 7B (Audio ON) Full Probe
# =============================================================================
# One-click script: installs all dependencies, downloads weights, runs
# full evaluation (9 tasks × 300 samples = 2700 probes), computes metrics.
#
# Usage:
#   bash scripts/run_probe_video_salmonn2.sh                # Full run (2700)
#   LIMIT=2 bash scripts/run_probe_video_salmonn2.sh        # Quick smoke test
#   NUM_GPUS=4 bash scripts/run_probe_video_salmonn2.sh     # Custom GPU count
#
# Environment:
#   - Requires: CUDA GPU, flash-attn, liger-kernel, torchcodec
#   - Uses transformers 4.51.3 shim (installed to /path/to/salmonn_pkgs)
#   - Model weights: local path or HuggingFace cache
# =============================================================================

set -e

MODEL="video-salmonn2+"
MODEL_TAG="Video-SALMONN2plus-7B"
MODEL_HF="tsinghua-ee/video-SALMONN2_plus_7B_full"
...
echo "==========================================" | tee -a "$LOG"
echo "  HF model   : $MODEL_HF"                   | tee -a "$LOG"
...
# Install transformers 4.51 shim (Video-SALMONN 2+ needs >=4.48, <4.52)
...
# Patch modeling_bert.py if needed (apply_chunking_to_forward compat)
BERT_FILE="${BENCH_DIR}/third_party/video-SALMONN-2/video_SALMONN2_plus/qwenvl/model/modeling_bert.py"
if [ -f "$BERT_FILE" ] && grep -q "apply_chunking_to_forward" "$BERT_FILE" 2>/dev/null; then
    echo "  Checking modeling_bert.py compatibility..." | tee -a "$LOG"
    # The shim transformers 4.51 has this function, so no patch needed
fi
...
echo "  Limit: $LIMIT  GPUs: $NUM_GPUS" | tee -a "$LOG"
